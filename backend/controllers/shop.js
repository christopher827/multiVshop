...
try {
      ...
      //   message: `Hello ${seller.name}, thanks for opening a shop with us`,
      // });
      res.status(201).json({
        success: true,
...
router.post("/create-shop2",async (req, res, next) => {
  try {
...
const newShop=await Shop.create(seller)
    ...
    try {
      ...
      res.status(201).json({
        success: true,
        message: `Dear ${seller.email} your shop has been created!`,
      });
    } catch (error) {
      return next(new ErrorHandler(error.message, 500));
    }
